...
public class P002122 {
	...
	private static void alinea2(PrintStream out) {
		...
		try(Scanner sc = new Scanner(new File("./src/exame/events.txt"))){
			em = new EventManager("event");
			Client cl;
			Event event = null;
			while (sc.hasNextLine()) {
				...
				if(linha[0].equals("*")){
					linha = linha[1].split(",");
					cl = em.addClient(linha[0], linha[1]);
					event = em.addEvent(cl, LocalDate.parse(linha[2]));
				}else if(linha[0].equals("#")){
					linha = linha[1].split(",");
					if(linha[0].equals("Catering")) // linha[1] holds the enum
						event.addActivity(new Catering(Catering.Option.DRINKS_AND_SNACKS, Integer.parseInt(linha[2])));
					else if(linha[0].equals("Culture"))
						event.addActivity(new Culture(Culture.Option.ARCHITECTURAL_TOUR, Integer.parseInt(linha[2])));
					else if(linha[0].equals("Sport"))
						event.addActivity(new Sport(Sport.Modality.HIKING, Integer.parseInt(linha[2])));
				}
			}
